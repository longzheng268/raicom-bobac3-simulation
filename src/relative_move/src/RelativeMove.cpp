...
int8_t RelativeMove::Init(ros::NodeHandle& nh) {
  ...
  xPid_ = std::make_shared<rei_tools::ReiPID>(1.0, 0, 0.03);
  yPid_ = std::make_shared<rei_tools::ReiPID>(1.0, 0, 0.03);
  thetaPid_ = std::make_shared<rei_tools::ReiPID>(2.0, 0, 0.08);
  xPid_->setOutputLimit(0.5, 0.01);
  yPid_->setOutputLimit(0.5, 0.01);
  thetaPid_->setOutputLimit(1.0, 0.08);
