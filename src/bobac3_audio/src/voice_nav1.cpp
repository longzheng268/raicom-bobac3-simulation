...
struct Point   //定义一个名为Student的结构体
{
	...
	struct Point m_point[13]={{1.12,1.080,-0.012,1.000,"深圳馆","深圳，中国的科创中心","好的，这就带您去深圳馆"},{1.195,2.065,0.013,1.000,"上海","上海，中国的经济中心","好的，这就带您去上海馆"},{2.644,0.10,0.03,0.99,"北京","北京，中国的首都","好的，这就带您去北京馆"},{2.665,1.077,-0.026,1.000,"广州","广州，自古以来都是中国的商都","好的，这就带您去广州馆"},{2.74,2.04,-0.028,0.999,"吉林","吉林，位于中国的东北是人参之都","好的，这就带您去吉林馆"},{0.10, -0.04,-0.000, 1.000,"起点","已回到起点"},{0.445, 2.1, 0.000, 0.995,"充电桩","已经到达充电桩"},{1.185,1.080,-0.012,1.000,"深圳馆","深圳，中国的科创中心","好的，这就带您去深圳馆"},{1.195,2.065,0.013,1.000,"上海","上海，中国的经济中心","好的，这就带您去上海馆"},{2.59,0.10,0.03,0.99,"北京","北京，中国的首都","好的，这就带您去北京馆"},{2.64,1.077,-0.026,1.000,"广州","广州，自古以来都是中国的商都","好的，这就带您去广州馆"},{2.72,2.04,-0.028,0.999,"吉林","吉林，位于中国的东北是人参之都","好的，这就带您去吉林馆"},{-0.051, -0.03,-0.000, 1.000,"起点","已回到起点"}};
...
int main(int argc,char **argv){
    ...
    while(ros::ok()){
            ...
            if(text.find("元宝") != string::npos){ //识别到“导航”关键词
                for(int i=7;i<12;i++){ //遍历所有参数
			...
			audio.goto_nav(&m_point[12]); //导航到匹配的导航点
                        audio.voice_tts(m_point[12].present.c_str()); //介绍导航语
                        break;
                    }
                }
            }
            if(text.find("一圈") != string::npos){ //识别到“带我参观一周”关键词
			...
			audio.goto_nav(&m_point[2]); //导航到匹配的导航点
      			...
      			audio.goto_nav(&m_point[5]); //导航到匹配的导航点
                        audio.voice_tts(m_point[5].present.c_str()); //介绍导航语
		}
    }
    return 0;
}
