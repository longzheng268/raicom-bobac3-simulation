...
int8_t ArPoseAdjust::Init(ros::NodeHandle& nh, bool getMarkerFromTopic) {
  nh_ = nh;
  ...
  // xPid_ = std::make_shared<rei_tools::ReiPID>(1.0, 0, 0.00);
  // yPid_ = std::make_shared<rei_tools::ReiPID>(1.0, 0, 0.00);
  // thetaPid_ = std::make_shared<rei_tools::ReiPID>(2.0, 0, 0.00);
  // xPid_->setOutputLimit(0.3, 0.05);
  // yPid_->setOutputLimit(0.3, 0.05);
  // thetaPid_->setOutputLimit(1.0, 0.1);

  // velPub_ = nh_.advertise<geometry_msgs::Twist>("cmd_vel", 5);
  // xErrPub_ = nh_.advertise<std_msgs::Float64>("ar_pose/x_err", 5);
  // yErrPub_ = nh_.advertise<std_msgs::Float64>("ar_pose/y_err", 5);
  // thetaErrPub_ = nh_.advertise<std_msgs::Float64>("ar_pose/theta_err", 5);
  expectXErr_ = 0.01;
  expectYErr_ = 0.01;
  expectThetaErr_ = 0.05;
  // markerSub_ =
  //     nh_.subscribe("ar_pose_marker", 1, &ArPoseAdjust::MarkerSubCb, this);
  return 0;
}
...
geometry_msgs::Twist ArPoseAdjust::GetTargetAheadPose(
  ...
  inTransform.rotation.x = -0.5;
  inTransform.rotation.y = -0.5;
  inTransform.rotation.z = -0.5;
  inTransform.rotation.w = 0.5;
